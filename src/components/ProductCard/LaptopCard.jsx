  ...
  const handleQuickAdd = async (e) => {
    ...
    try {
      ...
      const existingItem = cartItems.find(
        (item) => String(item.productId) === String(product.id) && item.fromTable === "LaptopUser"
      );
      ...
      if (existingItem) {
        ...
        await fetch("http://localhost:3000/cart", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            id: `cart-laptop-${product.id}-${Date.now()}`,
        ...
        <div className="card-gift-bonus">
          <div className="gift-bonus-line">
            ...
            <span className="gift-text-truncate">
              🔥 KM: Tặng Balo + Chuột gaming... và 2 quà khủng
            </span>
          </div>
      ...
      <div className="card-layer-5">
        <div className="card-partner-offer">
          ...
          <span className="partner-offer-text">
            🔥 Giảm thêm tới 1.000.000đ khi mở thẻ
          </span>
        </div>
      </div>
        ...
        <div className="stock-rating-row">
          <div className="stock-rating-left">
            ...
            <span className="rating-star-compact">
              ...
              <span className="review-num">({product?.reviewsCount || (Number(product?.id) || 1) * 6 + 18})</span>
            </span>
          </div>
          <button
            type="button"
